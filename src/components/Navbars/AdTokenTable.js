...
import "assets/css/components/navbars/AdTokenTable.scss"
import axios from "axios";
import { useDispatch } from "react-redux";
import { save_othernetworth_data } from "redux/actions/provider";
...
export default function AdTokenTable() {
    ...
    const getTokenList = async () => {
        ...
        if (TokenListDatas.data.succcess === true) {
            ...
            for (let i = 0; i < TokenListData.length; i++) {
                ...
                if (TokenListData[i].tokenName !== "" && TokenListData[i].tokenAmount.uiAmount > 0) {
                    ...
                    if (TokenListData[i].priceUsdt) {
                        ...
                        otherNetworth = otherNetworth + (TokenChip.Balance * TokenListData[i].priceUsdt);
                    }
                    else {
                        ...
                        if (marketData.data.data) {
                            ...
                            if (findUsdc[0]) {
                                if (findUsdc[0].price) {
                                    ...
                                    otherNetworth = otherNetworth + (TokenChip.Balance * TokenChip.Price);
                                }
                                else {
                                    TokenChip.Price = 0;
                                    TokenChip.Value = 0;
            ...
            dispatch(save_othernetworth_data(otherNetworth));
        }
    }
